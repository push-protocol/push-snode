...
export class PushSdkUtil {
  public static async checkPushInitDidSignature(
    ...
    const recoverAddrStr = verifyMessage(hashBytes, sig).toLowerCase()
    if (recoverAddrStr !== masterAddrStr) {
      return SigCheck.failWithText(
        `masterPublicKey address ${masterAddrStr} differs from signature addr ${recoverAddrStr}`
      )
    }
    return SigCheck.ok()
  }

  public static async checkPushNetworkSignature(
    ...
    if (caipNamespace === 'push') {
      ...
      if (!valid) {
        return SigCheck.failWithText(
          `sender address ${caipAddr} does not match recovered address ${recoveredAddr} signature was: ${sig}`
        )
      }
      return SigCheck.ok()
      ...
      if (!valid) {
        return SigCheck.failWithText(
          `sender address ${caipAddr} does not match with signature: ${sig}`
        )
      }
      return SigCheck.ok()
    } else {
  ...
  public static messageBytesToHashBytes(payload: Uint8Array): Uint8Array {
    const txSha = sha256(payload) // raw bytes (non ascii)
