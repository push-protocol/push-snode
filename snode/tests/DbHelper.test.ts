...
describe('DbHelpers tests', function () {
    it('test 8 bit for 1 key ', function () {
        let shardId = DbHelper.calculateShardForNamespaceIndex("feeds", "store0001");
    ...
    it('test 8 bit for 10 random keys', function () {
        for (let i = 0; i < 10; i++) {
            let shardId = DbHelper.calculateShardForNamespaceIndex(crypto.randomBytes(20).toString('hex'), crypto.randomBytes(20).toString('hex'));
            expect(shardId).lessThanOrEqual(255);
            expect(shardId).greaterThanOrEqual(0)
            console.log(shardId);
        }
